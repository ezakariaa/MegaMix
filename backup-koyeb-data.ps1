# Script PowerShell pour sauvegarder les donnees depuis Koyeb
# Ce script exporte les donnees (albums, tracks, artists) depuis le backend Koyeb

$ErrorActionPreference = "Stop"

Write-Host "Sauvegarde des donnees depuis Koyeb..." -ForegroundColor Cyan
...
$backupDir = "backups\koyeb-$timestamp"
...
$exportUrl = "$koyebUrl/api/music/export-data"

Write-Host "Telechargement des donnees depuis Koyeb..." -ForegroundColor Yellow
...
try {
    ...
    if ($response.success) {
        ...
        Write-Host ""
        Write-Host "Vous pouvez maintenant redployer en toute securite." -ForegroundColor White
        Write-Host "Pour restaurer, utilisez: .\restore-koyeb-data.ps1 -BackupPath `"$backupDir`"" -ForegroundColor Yellow
    } else {
